...
use crate::{
    registry::RuleLanguage, AnalyzerSignal, Phases, QueryMatch, Rule, RuleFilter, RuleGroup,
    ServiceBag,
};
...
impl GroupKey {
    ...
    pub fn group<G: RuleGroup<L>, L: Language>() -> Self {
        Self::new(G::NAME)
    }
}
...
impl RuleKey {
    ...
    pub fn rule<G: RuleGroup<RuleLanguage<R>>, R: Rule>() -> Self {
...
mod tests {
    ...
    fn suppressions() {
        ...
        fn parse_suppression_comment(comment: &str) -> Vec<(&str, Option<&str>)> {
            comment
                .trim_start_matches("//")
                .split(' ')
                .map(|item| ("lint", Some(item)))
                .collect()
        }
